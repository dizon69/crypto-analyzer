  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {assets.map((coin) => (
          <div key={coin.id} className="transition-transform transform hover:scale-105">
            <div className="bg-white shadow-lg rounded-2xl p-4 border border-gray-200 flex flex-col hover:shadow-xl transition duration-300">
              ...
              <div className="mt-4 text-lg font-bold text-gray-800">
                Rp {coin.current_price?.toLocaleString("id-ID") ?? "N/A"}
              </div>

              <div className="text-sm text-gray-600 mt-1">
                Volume: Rp {(coin.total_volume ?? 0).toLocaleString("id-ID")}
              </div>
